<?php function drawTrainersPage(Session $session, array $trainers, bool $isAdmin = false, array $adminData = []): void {
    ...
    $allGyms    = array_keys($allGyms);    sort($allGyms);
    $allClasses = array_keys($allClasses); sort($allClasses);
?>
...
<main class="trainers-page">
    ...
    <div class="admin-header">
        <div>
            ...
            <p class="admin-sub"><?= count($trainers) ?> trainer<?= count($trainers) !== 1 ? 's' : '' ?> registered</p>
        </div>
    ...
    <div class="admin-form-card" id="trainerForm" <?= ($editTrainer || $error) ? '' : 'hidden' ?>>
        ...
        <form method="POST" enctype="multipart/form-data" class="admin-form-grid">
            ...
            <div class="admin-field">
                ...
                <input type="text" name="first_name" required value="<?= htmlspecialchars($editTrainer['first_name'] ?? '') ?>">
            </div>
            <div class="admin-field">
                <label>Last Name</label>
                <input type="text" name="last_name" required value="<?= htmlspecialchars($editTrainer['last_name'] ?? '') ?>">
            </div>
            <div class="admin-field">
                <label>Email</label>
                <input type="email" name="email" required value="<?= htmlspecialchars($editTrainer['email'] ?? '') ?>">
            </div>
            <?php if (!$editTrainer): ?>
            <div class="admin-field">
            ...
            <div class="admin-field admin-field--full">
                ...
                <textarea name="certifications" rows="2"><?= htmlspecialchars($editTrainer['certifications'] ?? '') ?></textarea>
            </div>
            <div class="admin-field admin-field--full">
    ...
    <section class="trainers-grid">
        ...
        ?>
            ...
            <?php endif; ?>
                ...
                <img class="trainer-photo" src="<?= htmlspecialchars($photo) ?>" alt="<?= htmlspecialchars($fullName) ?>">
